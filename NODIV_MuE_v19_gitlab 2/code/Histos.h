...
namespace MuE {
  ...
  public:
    Histos(const MCpara & pargen, const AN_Input & paran, const TString & namePrefix="", bool debug=false);
    virtual ~Histos(){};
    void Fill(const Event & event, const MuEana & a);
    void FillTemplatesLO(const MuEana & a);
    void FillTemplatesNLO(const Event & event, const MuEana & a);
    inline void SetSums(const MCstat & s) {mcsums = s;}
    void Normalize(Long64_t n_events);
    ...
    void Do_Ratios();
    void RatioFinal(Long64_t n_events, TFile *fp, TString hname);
    void LoadExtHistos(TFile *fp);
    void LoadCarlos(const std::vector<std::pair<std::string,Long64_t> > & mc_inputs);
  ...
  private:
    ...
    const double & thetaMax;         // max theta for selection of events and template histograms
