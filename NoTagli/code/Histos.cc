...
////////////////////////////////////////////////////////////////////////////////////////////////
// FORTRAN Subroutine needed for reweighting the NLO events
//
// Inputs: 
//  Qmu : muon charge (+1 or -1)
//  p1  : incoming muon     fourmomentum (E,px,py,pz)
//  p2  : incoming electron       "
//  p3  : outgoing muon           "
//  p4  : outgoing electron       "
//  k   : outgoing photon         "      (if there)
//
// Outputs:
//  cm2 : coefficient of the pure-muon term (photon radiated from the muon leg)
//  ce2 : coefficient of the pure-electron term (photon radiated from the electron leg)
//  cem : coefficient of the interference term
//
extern "C" void muerad_(double *pQmu,double *pp1,double *pp2,double *pp3,double *pp4,double *pk,
			double *pcm2,double *pce2,double *pcem);
////////////////////////////////////////////////////////////////////////////////////////////////

Histos::Histos(const MuE::MCpara & pargen, const AN_Input & paran, const TString & namePrefix, bool debug):
  Emin_e(pargen.Emin_e), charge_mu(pargen.charge_mu), mass_mu(pargen.mass_mu), mass_e(pargen.mass_e),
  UNWGT(pargen.UNWGT), Wnorm(pargen.Wnorm),
  angleCorrPlots_finebins(paran.angleCorrPlots_finebins), doTemplates(paran.doTemplates), thetaMax(paran.thetaMax),
  ...
  prefix(namePrefix)
{
  ...
  const Double_t thetaMax_l = thetaMax;
...
void Histos::FillTemplatesNLO(const MuE::Event & event, const MuE::MuEana & a) {
  ...
  // currently there can be 0 or 1 photon in the final state
  auto n_photons = event.photons.size();
  if (n_photons > 1) {
    cout<<"\n *** ERROR: found more than one photon in "<<endl;
    cout<<"\t Run = "<<event.RunNr << ", Event = "<< event.EventNr <<endl;
    exit(10);
  }
  // number of final state particles at GEN level
  unsigned int npart = n_photons!=0 ? 3 : 2 ;

  // NLO event weight for Delta(alpha)=0 (no running) 
  double w0 = a.wgt_norun;

  //**** the NLO coefficients ce2, cm2 and cem can be calculated only once:
  double cm2,ce2,cem; // photon emission from muon line, electron line, interference 

  if (npart == 3) {
    // events with one real photon radiated:
    Double_t emuin = event.E_mu_in;
    Double_t pmuin = sqrt(emuin*emuin - mass_mu*mass_mu);
    
    double p1[4] = {emuin, 0., 0., pmuin};
    double p2[4] = {mass_e, 0., 0., 0.};
    double p3[4] = {event.P_mu_out.E, event.P_mu_out.px, event.P_mu_out.py, event.P_mu_out.pz};
    double p4[4] = {event.P_e_out.E, event.P_e_out.px, event.P_e_out.py, event.P_e_out.pz};
    
    double kg[4] = {0,0,0,0};
    if (n_photons >0) {
      kg[0] = event.photons[0].E;
      kg[1] = event.photons[0].px;
      kg[2] = event.photons[0].py;
      kg[3] = event.photons[0].pz;    
    }

    // compute NLO coefficients for reweighting (FORTRAN code by C.Carloni)
    double charge = charge_mu;
    muerad_(&charge, p1, p2, p3, p4, kg, &cm2, &ce2, &cem);
  }
  ...
  for (unsigned int iK=0; iK < ngrid; ++iK) {
    for (unsigned int iM=0; iM < ngrid; ++iM) {
      ...
      double wgtRew; // reweighting factor
      if (npart==3) {
	double dal13 = dalep(a.genKin.t13);
	double dal24 = dalep(a.genKin.t24);
	double dah13 = dahadPar(iparam, a.genKin.t13, par);
	double dah24 = dahadPar(iparam, a.genKin.t24, par);
	double da13 = dal13 + dah13;
	double da24 = dal24 + dah24;
	wgtRew = w0*( cm2/(1.-da24)/(1.-da24) + ce2/(1.-da13)/(1.-da13) + cem/(1.-da13)/(1.-da24) );
      }
      else {
	double dal = dalep(a.genKin.t13);
	double dah = dahadPar(iparam, a.genKin.t13, par);   
	double da  = dal + dah;
	wgtRew = w0/(1.-da)/(1.-da);
      }
...
void Histos::Fill(const MuE::Event & event, const MuE::MuEana & a) {
  ...
  //--------------------------------------------
  // fill NLO templates
  if (doTemplates) FillTemplatesNLO(event, a);
  //--------------------------------------------
...
void Histos::Normalize(Long64_t n_events) {
  //
  // normalize histograms to cross section
  //
  if (n_events<0) return;

  Long64_t nevtot = n_events != 0 ? n_events : mcsums.Nevgen;
...
void Histos::PlotResolutions() {
  ...
  TCanvas c_eres1(prefix+"c_eres1", "Resolution - Electron 1 GeV", 700, 500);
  c_eres1.Divide(2,1);
  c_eres1.cd(1);
  h_resol_thex->Draw();
  c_eres1.cd(2);
  h_resol_they->Draw();
  c_eres1.SaveAs(prefix+"resolution_e_1GeV.png");

  TCanvas c_eres5(prefix+"c_eres5", "Resolution - Electron 5 GeV", 700, 500);
  c_eres5.Divide(2,1);
  c_eres5.cd(1);
  h_resol_thex5->Draw();
  c_eres5.cd(2);
  h_resol_they5->Draw();
  c_eres5.SaveAs(prefix+"resolution_e_5GeV.png");

  TCanvas c_mures(prefix+"c_mures", "Resolution - Muon 148-150 GeV", 700, 500);
  c_mures.Divide(2,1);
  c_mures.cd(1);
  h_resol_thmux->Draw();
  c_mures.cd(2);
  h_resol_thmuy->Draw();
  c_mures.SaveAs(prefix+"resolution_mu_148-150GeV.png");
  ...
  TCanvas c_eresVsE(prefix+"c_eresVsE", "Resolution vs E - electron", 700, 700);
  c_eresVsE.Divide(1,2);
  c_eresVsE.cd(1);
  h_resol_thexVsE_sigma->Draw();
  c_eresVsE.cd(2);
  h_resol_theyVsE_sigma->Draw();
  c_eresVsE.SaveAs(prefix+"resolution_vs_Energy_electron.png");
  ...
  TCanvas c_muresVsE(prefix+"c_muresVsE", "Resolution vs E - muon", 700, 700);
  c_muresVsE.Divide(1,2);
  c_muresVsE.cd(1);
  h_resol_thmuxVsE_sigma->Draw();
  c_muresVsE.cd(2);
  h_resol_thmuyVsE_sigma->Draw();
  c_muresVsE.SaveAs(prefix+"resolution_vs_Energy_muon.png");
}
...
void Histos::Plot() {
  ...
  TCanvas c_t13 (prefix+"c_t13", "t13", 700, 500);
  c_t13.SetLogy();
  //h_t13->GetXaxis()->SetTitle("t_{13} (GeV^2)");
  hdraw(h_t13);
  c_t13.SaveAs(prefix+"t13.png");
  
  TCanvas c_t24 (prefix+"c_t24", "t24", 700, 500);
  c_t24.SetLogy();
  hdraw(h_t24);
  c_t24.SaveAs(prefix+"t24.png");
  
  TCanvas c_Ee (prefix+"c_Ee", "Electron energy", 700, 500);
  c_Ee.SetLogy();
  hdraw(h_Ee);
  c_Ee.SaveAs(prefix+"Ee.png");
  
  TCanvas c_Emu (prefix+"c_Emu", "Muon energy", 700, 500);
  c_Emu.SetLogy();
  hdraw(h_Emu);
  c_Emu.SaveAs(prefix+"Emu.png");
  
  TCanvas c_the (prefix+"c_the", "Electron angle", 700, 500);
  hdraw(h_the);
  c_the.SaveAs(prefix+"the.png");
  
  TCanvas c_thmu (prefix+"c_thmu", "Muon angle", 700, 500);
  c_thmu.SetLogy();
  hdraw(h_thmu);
  c_thmu.SaveAs(prefix+"thmu.png");
  
  TCanvas c_egamma (prefix+"c_egamma", "Photon energy", 700, 500);
  c_egamma.SetLogy();
  hdraw(h_egamma);
  c_egamma.SaveAs(prefix+"egamma.png");
  
  TCanvas c_thgamma (prefix+"c_thgamma", "Photon angle", 700, 500);
  hdraw(h_thgamma);
  c_thgamma.SaveAs(prefix+"thgamma.png");
...
void Histos::Plot2D(bool debug) {
  ...
  gStyle->SetOptStat(11);
  TCanvas cz_thmuVsthe(prefix+"cz_thmuVsthe", "Muon vs Electron angle", 700, 500);
  hsn_thmuVsthe_NLO_ref->Draw("COLZ");
  cz_thmuVsthe.SaveAs(prefix+"thmuVsthe_det_colz.png");

  TCanvas cz_thL_vs_thR(prefix+"cz_thL_vs_thR", "Theta_L vs Theta_R angle", 700, 700);
  hsn_thL_vs_thR_NLO_ref->Draw("COLZ");
  cz_thL_vs_thR.SaveAs(prefix+"thL_vs_thR_det_colz.png");
}
...
//
void Histos::RatioFinal(Long64_t n_events, TFile *fp, TString hname) {
...
void Histos::LoadCarlos(const vector<pair<string,Long64_t> > & mc_inputs) {
  string line;
