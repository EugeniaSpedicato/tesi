...
TH2F  *Th_E_el  = new TH2F("ThEel" , " Theta el Vs. E_ECAL",120,0,60,280,0.2,140);
TH2F  *Th_E_el1  = new TH2F("ThEel1" , " Theta el Vs. E_ECAL TAR 1",120,0,60,280,0.2,140);
TH2F  *Th_E_el2  = new TH2F("ThEel2" , " Theta el Vs. E_ECAL TAR 2",120,0,60,280,0.2,140);
...
double r_mu=sqrt((detKinBeamRot_x_in*detKinBeamRot_x_in)+(detKinBeamRot_y_in*detKinBeamRot_y_in));   
      ...
      if (r_mu<5 && E_ECAL>1 && abs(detKinBeamRot_cooXe) < 0.07125 && abs(detKinBeamRot_cooYe) < 0.07125 ) 
{
  ...
  // per muone          
    ...
    //Double_t detKinBeamRot_Emu=sqrt(Pmu*Pmu+mmu*mmu);
